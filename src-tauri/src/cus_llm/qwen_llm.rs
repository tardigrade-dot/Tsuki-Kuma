...
use candle::{CudaDevice, DType, Device, MetalDevice, Tensor};
...
impl TextGeneration {
    ...
    fn run(&mut self, prompt: &str, sample_len: usize) -> Result<String> {
        ...
        for index in 0..sample_len {
            ...
            self.tokenizer.next_token(next_token)?;
        }

        let result = self.tokenizer.decode_all().map_err(E::msg)?;
        println!("generated result: {result}");

        let rest = self.tokenizer.decode_rest().map_err(E::msg)?.unwrap();

        println!("generated rest: {rest}");
